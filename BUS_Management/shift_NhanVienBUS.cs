...
{
    public class shift_NhanVienBUS
    {
        ...
        private shift_NhanVienDAL shift_NhanVienDAL = new shift_NhanVienDAL();

        public DataTable getIdName_ByCaThuID(int Ca, string Thu, string idNV)
        {
            return this.shift_NhanVienDAL.getIdName_ByCaThuID(Ca, Thu, idNV);
        }
        public DataTable getShift()
        {
            return this.shift_NhanVienDAL.getShift();
        }
        public DataTable getALLShift_NhanVien_New()
        {
            return this.shift_NhanVienDAL.getALLShift_NhanVien_New();
        }

        public DataTable getIdName_ByCaThu(int Ca, string Thu)
        {
            return this.shift_NhanVienDAL.getIdName_ByCaThu(Ca, Thu);
        }

        //Gán bảng new qua bảng show
        public bool InsertIntoOld_FormNew()
        {
            return this.shift_NhanVienDAL.InsertIntoOld_FormNew();
        }

        //Reset new 
        public bool ResetShift_NhanVien_New()
        {
            return this.shift_NhanVienDAL.ResetShift_NhanVien_New();
        }

        //Reset
        public bool ResetShift_NhanVien()
        {
            return this.shift_NhanVienDAL.ResetShift_NhanVien();
        }

        public DataTable getALLShift_NhanVien()
        {
            return this.shift_NhanVienDAL.getALLShift_NhanVien();
        }

        public void chiaCaNhanVien()
        {
            DataTable table = this.nhanvienDAL.getNv_typeTho("Nhân Viên");
            int SoNV = this.nhanvienDAL.countEmployee_byType("Nhân Viên");
            int SoNVTrongCa = 3;
            ...
            for (int i = 0; i < Result.Count; i++)//thứ
            {
                for (int j = 0; j < Result[i].Count; j++)//slca
                {
                    for (int k = 0; k < Result[i][j].Count; k++)//slnvtc
                    {
                        ...
                        shift_NhanVienDTO shift_NhanVienDTO = dto(id_NV, thu, ca);
                        this.shift_NhanVienDAL.insertShift(shift_NhanVienDTO);
        ...
        private shift_NhanVienDTO dto(string id_nv, int thu, int ca)
        {
            shift_NhanVienDTO shift_NhanVienDTO;
            if (thu == 2)
            {
                shift_NhanVienDTO = new shift_NhanVienDTO(id_nv, ca, 0, 0, 0, 0, 0, 0);
            }
            else if (thu == 3)
            {
                shift_NhanVienDTO = new shift_NhanVienDTO(id_nv, 0, ca, 0, 0, 0, 0, 0);
            }
            else if (thu == 4)
            {
                shift_NhanVienDTO = new shift_NhanVienDTO(id_nv, 0, 0, ca, 0, 0, 0, 0);
            }
            else if (thu == 5)
            {
                shift_NhanVienDTO = new shift_NhanVienDTO(id_nv, 0, 0, 0, ca, 0, 0, 0);
            }
            else if (thu == 6)
            {
                shift_NhanVienDTO = new shift_NhanVienDTO(id_nv, 0, 0, 0, 0, ca, 0, 0);
            }
            else if (thu == 7)
            {
                shift_NhanVienDTO = new shift_NhanVienDTO(id_nv, 0, 0, 0, 0, 0, ca, 0);
            }
            else
            {
                shift_NhanVienDTO = new shift_NhanVienDTO(id_nv, 0, 0, 0, 0, 0, 0, ca);
            }
            return shift_NhanVienDTO;
        }
